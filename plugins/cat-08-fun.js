...
module.exports = {
  ...
  usage: '.pair <phone_number>  e.g.  .pair 923442174513',
  ownerOnly: false,
    ...
    async handler(sock, message, args, context) {
      ...
      try {
        ...
        try {
          ...
          if (data?.status && typeof data.result === 'string') imageUrl = data.result;
        } catch (_) {}

        // Fallback: public Danbooru API, restricted to safe Blue Archive posts.
        if (!imageUrl) {
          const { data } = await axios.get('https://danbooru.donmai.us/posts.json', {
            params: { tags: 'blue_archive rating:safe', limit: 20 },
            timeout: 15000,
            headers: { 'User-Agent': 'JUNAID-MD-321/1.0' }
          });
          const posts = Array.isArray(data) ? data.filter(x => x?.file_url || x?.large_file_url) : [];
          if (!posts.length) throw new Error('No Blue Archive image was found');
          const post = posts[Math.floor(Math.random() * posts.length)];
          imageUrl = post.file_url || post.large_file_url;
        }

        await sendMedia(sock, chatId, message, imageUrl, 'image', '🎮 Blue Archive');
      } catch (err) {
        await sock.sendMessage(chatId, { text: `❌ .ba failed: ${err.message || 'Unable to fetch image'}` }, { quoted: message });
      }
    }
  },
  {
