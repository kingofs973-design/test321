    ...
    try {
      ...
      try {
        const res = await axios.get(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(formattedQuery)}`, {
          timeout: 15000,
          headers: {
            'User-Agent': 'JUNAID-MD-321/1.0 (Wikipedia command)',
            'Accept-Language': 'en'
          }
        });
        data = res.data;
      } catch (firstErr) {
        // Reliable fallback: MediaWiki search + page extract.
        const search = await axios.get('https://en.wikipedia.org/w/api.php', {
          ...
          headers: { 'User-Agent': 'JUNAID-MD-321/1.0 (Wikipedia command)' }
        });
        const hit = search.data?.query?.search?.[0];
        if (hit?.title) {
          const page = await axios.get('https://en.wikipedia.org/w/api.php', {
            timeout: 15000,
            params: { action: 'query', prop: 'extracts|info', exintro: 1, explaintext: 1, inprop: 'url', titles: hit.title, format: 'json', origin: '*' },
            headers: { 'User-Agent': 'JUNAID-MD-321/1.0 (Wikipedia command)' }
          });
          const pages = page.data?.query?.pages || {};
          const item = Object.values(pages)[0];
          data = item ? { title: item.title, extract: item.extract, content_urls: { desktop: { page: item.fullurl || `https://en.wikipedia.org/wiki/${encodeURIComponent(item.title.replace(/ /g, '_'))}` } } } : null;
        }
      }

      if (data?.extract) {
        await sock.sendMessage(chatId, {
          text: `▢ *Wikipedia*\n\n‣ Search: ${data.title}\n\n${data.extract}\n\nRead more: ${data.content_urls?.desktop?.page || `https://en.wikipedia.org/wiki/${encodeURIComponent(data.title.replace(/ /g, '_'))}`}`,
          ...channelInfo
        }, { quoted: message });
      } else {
        await sock.sendMessage(chatId, { text: "⚠️ No Wikipedia result found. Try another search word.", ...channelInfo }, { quoted: message });
      }
    } catch (e) {
    ...
    for (const anime of animes.slice(0, 8)) {
        try {
            ...
            cards.push({
                ...
                footer: { text: '🌸 JUNAID MD 302 Anime Search' },
                nativeFlowMessage: {
                    buttons: [
                        {
                            name:             'cta_url',
    ...
    for (const photo of photos.slice(0, 8)) {
        try {
            ...
            cards.push({
                ...
                footer: { text: '🖼 JUNAID MD 302 Image Search' },
                nativeFlowMessage: {
                    buttons: [
                        {
                            name:             'cta_url',
        ...
        viewOnceMessage: {
            message: {
                interactiveMessage: {
                    ...
                    footer:          { text: 'Swipe ◀️▶️ • JUNAID MD 302' },
                    carouselMessage: { cards }
                }
            }
        }
    ...
    for (const book of books.slice(0, 8)) {
        try {
            ...
            cards.push({
                ...
                body:   { text: bodyText },
                footer: { text: '📚 JUNAID MD 302 Book Search' },
