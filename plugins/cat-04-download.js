'use strict';
// JUNAID MD 302 — Download Bundle v3 (upgraded APIs)
...
async function sendAudio(sock, chatId, message, buf, caption) {
  await sock.sendMessage(chatId, { audio: buf, mimetype: 'audio/mpeg', ptt: false, fileName: 'tiktok_audio.mp3', caption }, { quoted: message });
}

async function extractMp3(videoBuffer) {
  const fs = require('fs');
  const os = require('os');
  const path = require('path');
  const { spawn } = require('child_process');
  const ffmpegPath = process.env.FFMPEG_PATH || 'ffmpeg';
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'junaid-tt-'));
  const input = path.join(dir, 'input.mp4');
  const output = path.join(dir, 'output.mp3');
  fs.writeFileSync(input, videoBuffer);
  try {
    await new Promise((resolve, reject) => {
      const cp = spawn(ffmpegPath, ['-y', '-i', input, '-vn', '-codec:a', 'libmp3lame', '-b:a', '128k', output], { stdio: 'ignore' });
      cp.on('error', reject);
      cp.on('close', code => code === 0 ? resolve() : reject(new Error('ffmpeg exited with code ' + code)));
    });
...
async function tryTikwm(url) {
  ...
  return { url: videoUrl, title: d.title, author: d.author?.nickname, quality: d.hdplay ? 'HD' : 'SD', music: d.music || d.music_info?.play || d.music_info?.play_url };
}
...
async function tryDiscardApi(url) {
  ...
  return { url: videoUrl, title: res.title, author: res.author?.nickname, quality: hd ? 'HD No Watermark' : 'No Watermark', stats: res.stats, music: res.music_info?.play || res.music_info?.play_url || res.music_info?.url };
}
...
async function tryMrnima(url) {
  ...
  return { url: videoUrl, title: res?.title || 'TikTok Video', author: res?.author, music: res?.music || res?.music_url || res?.musicUrl };
}
...
async function tryRuhend(url) {
  ...
  return { url: videoUrl, title: res?.data?.title, music: res?.data?.music || res?.data?.music_url };
}
...
module.exports = {
  ...
  async handler(sock, message, args, context = {}) {
    ...
    // Show the same simple format picker as the user's example: no extra question.
    // The user chooses Music or Video with one tap.
    const senderId = context.senderId || message.key.participant || message.key.remoteJid;
    global._tiktokPending = global._tiktokPending || new Map();
    global._tiktokPending.set(`${chatId}:${senderId}`, { url, message, expires: Date.now() + 5 * 60 * 1000 });

    await sock.sendMessage(chatId, {
      text: `🎵 *TikTok Downloader*\n\n🔗 ${url}\n\nChoose how you want this TikTok:`,
      footer: 'JUNAID_MD_³⁰² • TikTok',
      buttons: [
        { buttonId: 'tiktok_audio', buttonText: { displayText: '🎵 Music' }, type: 1 },
        { buttonId: 'tiktok_video', buttonText: { displayText: '🎬 Video' }, type: 1 }
      ],
      headerType: 1
    }, { quoted: message });
  }
...
// Handle the Music / Video button selected by the user.
global._handleTikTokButton = async function(sock, message, mode, context = {}) {
  const chatId = context.chatId || message.key.remoteJid;
  const senderId = context.senderId || message.key.participant || message.key.remoteJid;
  const key = `${chatId}:${senderId}`;
  const pending = global._tiktokPending?.get(key);
  if (!pending || pending.expires < Date.now()) {
    global._tiktokPending?.delete(key);
    await sock.sendMessage(chatId, { text: '⚠️ This TikTok selection has expired. Send .tiktok <TikTok URL> again.' }, { quoted: message });
    return true;
  }
  global._tiktokPending.delete(key);
  const url = pending.url;
  await sock.sendMessage(chatId, { text: mode === 'audio' ? '⏳ Downloading TikTok music...' : '⏳ Downloading TikTok video...' }, { quoted: message });
  let lastErr;
  for (const m of METHODS) {
    try {
      ...
      if (mode === 'video') {
        ...
        let audioBuf = null;
        if (info.music) {
          try { audioBuf = await downloadBuffer(info.music, 60000); } catch {}
        }
        if (!audioBuf) audioBuf = await extractMp3(buf);
        await sendAudio(sock, chatId, message, audioBuf, caption);
      }
      return true;
    } catch (e) { lastErr = e; }
  }
  await sock.sendMessage(chatId, { text: `❌ TikTok ${mode} download failed: ${lastErr?.message || 'Unknown error'}` }, { quoted: message });
  return true;
};
