/*****************************************************************************
 *  plugins/antilink.js — JUNAID MD 302 ULTRA v6
...
async function readConfig(chatId) {
    try {
        ...
        if (c) {
            const cfg = { ...DEFAULT_CONFIG, ...c, types: { ...DEFAULT_CONFIG.types, ...(c.types || {}) } };
            // Anti-link policy requested for this bot: once enabled, a link is
            // deleted and the sender is removed immediately. Keep explicit
            // `off` respected, but migrate old warn/delete configs to kick.
            if (cfg.enabled && cfg.mode !== 'kick') cfg.mode = 'kick';
            return cfg;
        }
...
/* ─── Resolve sender to a removable participant JID ──────────────────────── */
async function resolveRemovableJid(sock, chatId, senderId) {
    const raw = String(senderId || '');
    try {
        const meta = await sock.groupMetadata(chatId);
        const parts = meta?.participants || [];
        const rawBare = raw.split('@')[0].split(':')[0];
        const match = parts.find(p => {
            const ids = [p.id, p.jid, p.lid, p.phoneNumber].filter(Boolean).map(v => String(v));
            return ids.some(v => {
                const bare = v.split('@')[0].split(':')[0];
                return v === raw || bare === rawBare;
            });
        });
        if (!match) return toSWJid(raw) || raw;
        // Prefer the real phone JID when Baileys exposes phoneNumber; this
        // avoids trying to remove an @lid as if it were a phone number.
        if (match.phoneNumber && String(match.phoneNumber).includes('@')) return String(match.phoneNumber);
        if (match.id && String(match.id).endsWith('@s.whatsapp.net')) return String(match.id);
        if (match.jid && String(match.jid).endsWith('@s.whatsapp.net')) return String(match.jid);
        return String(match.id || match.jid || match.lid || raw);
    } catch (e) {
        console.error('[ANTILINK] participant resolve failed:', e.message);
        return toSWJid(raw) || raw;
    }
}
...
async function handleLinkDetection(sock, chatId, message, userMessage, senderId) {
    try {
        ...
        const normSender  = await resolveRemovableJid(sock, chatId, senderId);
        ...
        // ── STEP 4: Immediate kick policy ─────────────────────────────────
        // The requested anti-link behavior is always: delete first, then
        // remove the member. This is deliberately after successful deletion.
        if (isBotAdmin) {
            try {
                await sock.groupParticipantsUpdate(chatId, [normSender], 'remove');
                await safeSend(sock, chatId, {
                    text: `🚫 @${senderShort} removed for sharing a link.`,
                    mentions: [normSender]
                });
            } catch (e) {
                console.error('[ANTILINK] immediate kick failed:', e.message);
                await safeSend(sock, chatId, {
                    text: `⚠️ @${senderShort} — link deleted, but I could not remove the member. Make sure I am a group admin.`,
                    mentions: [normSender]
                });
            }
        } else {
            await safeSend(sock, chatId, {
                text: `⚠️ @${senderShort} — link deleted, but I need admin rights to remove you.`,
                mentions: [normSender]
            });
        }
        return;

        /* legacy mode handlers intentionally disabled for this anti-link policy.
