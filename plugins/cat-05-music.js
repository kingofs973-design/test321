'use strict';
/*****************************************************************************
 *  cat-05-music.js — JUNAID MD 302 Music Bundle v5 ULTRA
 *  Developed By JUNAID MD 302
 *
...
/* ─── YouTube format picker helpers ─────────────────────────────────────── */
async function sendYouTubeChoice(sock, chatId, message, query, video) {
    ...
    return sock.sendMessage(chatId, {
        ...
        buttons: [
            { buttonId: `.ytaudio ${query}`, buttonText: { displayText: '🎵 Audio / MP3' }, type: 1 },
            { buttonId: `.ytvideo ${query}`, buttonText: { displayText: '🎬 Video / MP4' }, type: 1 },
            { buttonId: `.ytvoice ${query}`, buttonText: { displayText: '🎙️ Voice' }, type: 1 }
        ],
        headerType: 1
    }, { quoted: message });
}

async function resolveYouTubeQuery(query) {
    const isUrl = /^https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)\//i.test(query);
    return isUrl
        ? { url: query, title: query, timestamp: '?', author: { name: '' }, thumbnail: null }
        : await ytSearch(query);
}

async function sendYouTubeAudio(sock, message, query, asVoice = false) {
    const chatId = message.key.remoteJid;
    const video = await resolveYouTubeQuery(query);
    let localFile = null;
    try {
        // Prefer a local download: remote API URLs can expire or be rejected
        // by WhatsApp's media fetcher before the audio reaches the chat.
        try {
            const local = await downloadAudioLocal(video.url);
            localFile = local.path;
            await sock.sendMessage(chatId, {
                audio: require('fs').readFileSync(localFile),
                mimetype: 'audio/mpeg',
                fileName: `${(local.title || video.title || 'song').slice(0, 80)}.mp3`,
                ptt: !!asVoice
            }, { quoted: message });
            return;
        } catch (localErr) {
            console.log('[SONG] local audio failed, using API fallbacks:', localErr.message);
        }
        const dl = await downloadAny(video.url, 'mp3');
        await sock.sendMessage(chatId, {
            audio: { url: dl.downloadUrl },
            mimetype: 'audio/mpeg',
            fileName: `${(dl.title || video.title || 'song').slice(0, 80)}.mp3`,
            ptt: !!asVoice
        }, { quoted: message });
    } finally {
        if (localFile) { try { require('fs').unlinkSync(localFile); } catch (_) {} }
    }
}

/* ─── .song — YouTube picker: Audio / Video / Voice ─────────────────────── */
const _song = {
    command: 'song', aliases: ['mp3', 'audio', 'dlmp3'],
    category: 'music', description: 'Search YouTube and choose Audio, Video or Voice',
    ...
    async handler(sock, message, args, context = {}) {
        ...
        try {
            ...
            const video = await resolveYouTubeQuery(query);
            await sendYouTubeChoice(sock, chatId, message, query, video);
        } catch (e) {
...
/* ─── .ytaudio / .ytvoice — selected by the YouTube picker ─────────────── */
const _ytaudio = {
    command: 'ytaudio', aliases: [],
    category: 'music', description: 'Download selected YouTube song as MP3',
    usage: '.ytaudio <song name | YouTube URL>',
    async handler(sock, message, args, context = {}) {
        const query = args.join(' ').trim();
        if (!query) return sock.sendMessage(context.chatId || message.key.remoteJid, { text: '❌ Song name or YouTube link is required.' }, { quoted: message });
        try {
            await sock.sendMessage(context.chatId || message.key.remoteJid, { text: '🎵 *Downloading Audio / MP3...*' }, { quoted: message });
            await sendYouTubeAudio(sock, message, query, false);
        } catch (e) {
            await sock.sendMessage(context.chatId || message.key.remoteJid, { text: `❌ Audio download failed: ${e.message}` }, { quoted: message });
        }
    }
};

const _ytvoice = {
    command: 'ytvoice', aliases: [],
    category: 'music', description: 'Download selected YouTube song as WhatsApp Voice',
    usage: '.ytvoice <song name | YouTube URL>',
    async handler(sock, message, args, context = {}) {
        const query = args.join(' ').trim();
        if (!query) return sock.sendMessage(context.chatId || message.key.remoteJid, { text: '❌ Song name or YouTube link is required.' }, { quoted: message });
        try {
            await sock.sendMessage(context.chatId || message.key.remoteJid, { text: '🎙️ *Downloading Voice...*' }, { quoted: message });
            await sendYouTubeAudio(sock, message, query, true);
        } catch (e) {
            await sock.sendMessage(context.chatId || message.key.remoteJid, { text: `❌ Voice download failed: ${e.message}` }, { quoted: message });
        }
    }
};

/* ─── .ytvideo — selected by the YouTube picker ─────────────────────────── */
const _ytvideo = {
    command: 'ytvideo', aliases: [],
    category: 'download', description: 'Download selected YouTube song as MP4',
    usage: '.ytvideo <song name | YouTube URL>',
    async handler(sock, message, args, context = {}) {
        const chatId = context.chatId || message.key.remoteJid;
        const query = args.join(' ').trim();
        if (!query) return sock.sendMessage(chatId, { text: '❌ Song name or YouTube link is required.' }, { quoted: message });
        try {
            await sock.sendMessage(chatId, { text: '🎬 *Downloading Video / MP4...*' }, { quoted: message });
            const video = await resolveYouTubeQuery(query);
            let localFile = null;
            try {
                try {
                    const local = await require('../lib/ytdl').downloadVideo(video.url, 360);
                    localFile = local;
                    await sock.sendMessage(chatId, {
                        video: require('fs').readFileSync(localFile),
                        mimetype: 'video/mp4',
                        fileName: `${(video.title || 'video').slice(0, 80)}.mp4`,
                        caption: `🎬 *${video.title || 'Video'}*\n\n> *_JUNAID MD 302_*`
                    }, { quoted: message });
                    return;
                } catch (localErr) {
                    console.log('[SONG] local video failed, using API fallbacks:', localErr.message);
                }
                const videoData = await downloadAny(video.url, '360');
                await sock.sendMessage(chatId, {
                    video: { url: videoData.downloadUrl },
                    mimetype: 'video/mp4',
                    fileName: `${(videoData.title || video.title || 'video').slice(0, 80)}.mp4`,
                    caption: `🎬 *${videoData.title || video.title || 'Video'}*\n\n> *_JUNAID MD 302_*`
                }, { quoted: message });
            } finally {
                if (localFile) { try { require('fs').unlinkSync(localFile); } catch (_) {} }
            }
...
const _video = {
    command: 'video', aliases: ['ytmp4', 'ytdl'],
...
/* ─── Register all ──────────────────────────────────────────────────────────── */
const ALL = [_play, _play2, _song, _ytaudio, _ytvideo, _ytvoice, _video, _lyrics, _trending, _radio, _shazam, _spotify, _ringtone, _soundcloud];
for (const m of ALL) {
    _bundle.push(m);
    try {
