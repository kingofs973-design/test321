...
const OWNER_NAME = process.env.OWNER_NAME || config.OWNER_NAME || 'JUNAID MD 302';
const BOT_NAME   = process.env.BOT_NAME   || config.BOT_NAME   || 'JUNAID MD 302';
...
function getQuotedMessage(message) {
  ...
  return {
    key: {
      ...
      fromMe: false,
      id: ctx.stanzaId,
      participant: ctx.participant || message.key?.participant || undefined,
    },
...
async function uploadToCatbox(buffer, ext, mime) {
  ...
  const res = await axios.post('https://catbox.moe/user/api.php', form, {
    headers: { ...form.getHeaders(), 'User-Agent': 'JUNAID-MD-321/1.0' },
    timeout: 120000,
...
> 🔗 *${BOT_NAME} Media Uploader*`
      ...
      try {
        ...
        console.warn('[TOURL] primary media download failed:', downloadErr.message);
      }

      // Fallback for quoted media / Baileys message variants.
      if (!buffer?.length) {
        const { downloadContentFromMessage } = require('@whiskeysockets/baileys');
        const mm = targetMsg.message || {};
        const mediaTypes = ['imageMessage','videoMessage','audioMessage','stickerMessage','documentMessage'];
        for (const key of mediaTypes) {
          const media = mm[key] || mm.documentWithCaptionMessage?.message?.documentMessage;
          if (!media) continue;
          try {
            const stream = await downloadContentFromMessage(media, key.replace('Message',''));
            const chunks = [];
            for await (const chunk of stream) chunks.push(chunk);
            buffer = Buffer.concat(chunks);
            if (buffer.length) break;
          } catch (fallbackErr) {
            console.warn(`[TOURL] fallback ${key} failed:`, fallbackErr.message);
          }
        }
      }

      if (!buffer?.length) throw new Error('Failed to download media. Reply directly to the media and try again.');
...
💡 *Try a smaller file*`
      ...
      // Use Catbox for every media type. This removes the external ImgBB API-key
      // dependency and avoids ImgBB 412/size-limit failures on Railway.
      let mediaUrl = '';
      let mediaType = 'File';
      let deleteUrl = '';

      mediaUrl = await uploadToCatbox(buffer, `.${type.ext}`, type.mime);
      if      (type.mime.startsWith('image/'))  mediaType = 'Image 🖼️';
      else if (type.mime.startsWith('video/'))  mediaType = 'Video 🎬';
      else if (type.mime.startsWith('audio/'))  mediaType = 'Audio 🎵';
      else if (type.mime === 'application/pdf') mediaType = 'PDF 📄';
      else if (type.mime === 'application/zip') mediaType = 'ZIP 📦';
    ...
    for (const article of articles.slice(0, 8)) {
        try {
            ...
            cards.push({
                ...
                footer: { text: '📡 JUNAID MD 302 News Search' },
                nativeFlowMessage: {
                    buttons: [
                        {
                            name:             'cta_url',
        ...
        viewOnceMessage: {
            message: {
                interactiveMessage: {
                    ...
                    footer:          { text: 'Swipe ◀️▶️ • JUNAID MD 302 News' },
...
try {
  const _m = (function() {
    ...
    function firstMatch(html, re) {
      ...
      return m ? m[1].replace(/<[^>]*>/g, '').replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').trim() : '';
    }
    ...
    module.exports = {
      ...
      category: 'tools',
      description: 'Analyze basic SEO information of a website',
      usage: '.seo <url>',
      ...
      async handler(sock, message, args, context = {}) {
        ...
        if (!url) {
          return await sock.sendMessage(chatId, { text: '🔍 *SEO Analyzer*\n\nUsage: .seo <website-url>\nExample: .seo example.com' }, { quoted: message });
        }
        if (!/^https?:\/\//i.test(url)) url = 'https://' + url;
        let parsed;
        try { parsed = new URL(url); } catch {
          return await sock.sendMessage(chatId, { text: '❌ Invalid website URL.\nExample: .seo example.com' }, { quoted: message });
        }
        ...
        try {
          const response = await axios.get(parsed.href, {
            ...
            responseType: 'text',
            headers: { 'User-Agent': 'Mozilla/5.0 (compatible; JUNAID-MD-321 SEO Bot/1.0)' },
            validateStatus: () => true
          });
          const html = String(response.data || '');
          if (!html || typeof response.data !== 'string') {
            return await sock.sendMessage(chatId, { text: `❌ Could not read the website. HTTP ${response.status}` }, { quoted: message });
          }

          const title = firstMatch(html, /<title[^>]*>([\s\S]*?)<\/title>/i) || 'Not found';
          const description = firstMatch(html, /<meta[^>]+name=["']description["'][^>]+content=["']([^"']*)["'][^>]*>/i) ||
                             firstMatch(html, /<meta[^>]+content=["']([^"']*)["'][^>]+name=["']description["'][^>]*>/i) || 'Not found';
          const canonical = firstMatch(html, /<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']+)["'][^>]*>/i) ||
                            firstMatch(html, /<link[^>]+href=["']([^"']+)["'][^>]+rel=["']canonical["'][^>]*>/i) || 'Not found';
          const robots = firstMatch(html, /<meta[^>]+name=["']robots["'][^>]+content=["']([^"']*)["'][^>]*>/i) ||
                         firstMatch(html, /<meta[^>]+content=["']([^"']*)["'][^>]+name=["']robots["'][^>]*>/i) || 'Not specified';
          ...
          const lang = firstMatch(html, /<html[^>]+lang=["']([^"']+)["']/i) || 'Not specified';
          const charset = firstMatch(html, /<meta[^>]+charset=["']?([^"' >;]+)/i) || firstMatch(html, /charset=([^"' >;]+)/i) || 'Not detected';
          const ogTitle = firstMatch(html, /<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']*)["']/i) ||
                          firstMatch(html, /<meta[^>]+content=["']([^"']*)["'][^>]+property=["']og:title["']/i) || 'Not found';
          const ogImage = firstMatch(html, /<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']*)["']/i) || 'Not found';
          const twitterCard = firstMatch(html, /<meta[^>]+name=["']twitter:card["'][^>]+content=["']([^"']*)["']/i) || 'Not found';
          const wordText = html.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/&[^;]+;/g, ' ');
          const words = (wordText.match(/\b[\p{L}\p{N}]+\b/gu) || []).length;
          const scoreItems = [
            ['Title', title !== 'Not found'], ['Description', description !== 'Not found'],
            ['H1', h1 === 1], ['Canonical', canonical !== 'Not found'], ['Viewport', viewport === 'Yes'],
            ['Robots', robots !== 'Not specified'], ['Language', lang !== 'Not specified']
          ];
          const score = Math.round(scoreItems.filter(x => x[1]).length / scoreItems.length * 100);

          let out = `🔎 *JUNAID-MD-321 SEO REPORT*\n\n`;
          out += `🌐 *URL:* ${esc(parsed.href)}\n`;
          out += `📡 *HTTP Status:* ${response.status}\n`;
          out += `📊 *Basic SEO Score:* ${score}/100\n\n`;
          out += `📝 *Meta*\n• Title: ${esc(title)}\n• Description: ${esc(description)}\n• Canonical: ${esc(canonical)}\n• Robots: ${esc(robots)}\n\n`;
          out += `🏗️ *Page Structure*\n• H1: ${h1}\n• H2: ${h2}\n• Links: ${links}\n• Images: ${images} (${imagesWithAlt} with alt)\n• Words: ${words}\n• Scripts: ${scripts}\n• Forms: ${forms}\n\n`;
          out += `📱 *Technical*\n• Viewport: ${viewport}\n• Language: ${esc(lang)}\n• Charset: ${esc(charset)}\n\n`;
          out += `📣 *Social Meta*\n• OG Title: ${esc(ogTitle)}\n• OG Image: ${esc(ogImage)}\n• Twitter Card: ${esc(twitterCard)}\n\n`;
          out += `💡 *Quick Tips*\n`;
          if (title === 'Not found') out += '• Add a unique <title>.\n';
          if (description === 'Not found') out += '• Add a meta description.\n';
          if (h1 !== 1) out += '• Keep one clear H1 heading.\n';
          if (images > imagesWithAlt) out += '• Add alt text to images.\n';
          if (viewport !== 'Yes') out += '• Add a mobile viewport meta tag.\n';
          if (canonical === 'Not found') out += '• Consider adding a canonical URL.\n';
          if (score === 100) out += '• Basic SEO checks look good. ✅\n';

          for (let i = 0; i < out.length; i += 6000) {
            await sock.sendMessage(chatId, { text: out.slice(i, i + 6000) }, { quoted: message });
          }
        } catch (error) {
          console.error('SEO plugin error:', error);
          const msg = error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT'
            ? '❌ Website request timed out.'
            : `❌ Could not analyze this website: ${error.message || 'request failed'}`;
