...
// Default values
const DEFAULT_BOT_NAME = settings.botName || 'JUNAID MD 302';
const DEFAULT_OWNER = settings.botOwner || 'JUNAID MD 302';
  ...
  async handler(sock, message, args, context) {
    ...
    const forwardContextInfo = {
      ...
      forwardedNewsletterMessageInfo: {
        ...
        newsletterName: config.botName || 'JUNAID MD 302',
        serverMessageId: -1
      }
    };
...
module.exports = {
    ...
    async handler(sock, message, args, context = {}) {
        ...
        try {
            ...
            if (args.length === 0) {
                return await sock.sendMessage(chatId, {
                    text: `🔄 *Auto Status – JUNAID MD 302*\n\n` +
