...
• \`.tts2 guide\` – Detailed instructions`,
        ...
        // ==================== SIMPLE URDU TTS ====================
        // If the user runs `.tts2 <text>` without a sub-command, treat the
        // whole message as Urdu text and return a voice note. This keeps the
        // command simple for group replies/mentions.
        const knownSubCommands = new Set(['guide', 'speak', 'speak-urdu', 'clone', 'list-voices', 'speak-cloned', 'delete']);
        if (!knownSubCommands.has(subCmd)) {
            const text = args.join(' ').trim();
            if (text) {
                try {
                    const url = `${GOOGLE_TTS_URL}?ie=UTF-8&q=${encodeURIComponent(text.slice(0, 200))}&tl=ur&client=tw-ob`;
                    ...
                    if (!audioBuffer.length) throw new Error('Empty TTS response');
                    return await sock.sendMessage(chatId, {
                        audio: audioBuffer,
                        mimetype: 'audio/mpeg',
                        ptt: true,
                    ...
                    console.error('[TTS2] Simple Urdu TTS error:', error.message);
                    return await logAndReply(`Urdu voice failed: ${error.message}`, true);
                }
            }
        }
        ...
        sections: [{
            ...
            children: [
                ...
                new Paragraph({
                    children: [
                        new TextRun({
                            text  : `Generated by JUNAID MD 302`,
                            color : '888888',
                            size  : 18,
                            italics: true,
                        }),
...
try {
  const _m = (function() {
    const module = {exports: {}}; const exports = module.exports;
    // Reliable local HD enhancer: works with replied/sent images and does not depend on an external HD API.
    const sharp = require('sharp');
    const { downloadContentFromMessage } = require('@whiskeysockets/baileys');

    async function imageBufferFromMessage(msg) {
      if (!msg) return null;
      const image = msg.imageMessage || msg;
    ...
    module.exports = {
      ...
      category: 'tools',
      description: 'Enhance and upscale an image to HD',
      usage: 'Reply to an image with .hd or send an image with caption .hd',
      ...
      async handler(sock, message, args, context = {}) {
        ...
        try {
          ...
          if (quoted?.imageMessage) input = await imageBufferFromMessage(quoted);
          if (!input && message.message?.imageMessage) input = await imageBufferFromMessage(message.message);

          // URL support is kept for compatibility.
          if (!input && args.length) {
            const url = args.join(' ').trim();
            if (/^https?:\/\//i.test(url)) {
              const axios = require('axios');
              const r = await axios.get(url, { responseType: 'arraybuffer', timeout: 30000, maxContentLength: Infinity });
          ...
          if (!input) {
            return await sock.sendMessage(chatId, {
              text: '📸 *HD Image Enhancer*\n\nReply to an image with `.hd`, or send an image with the caption `.hd`.\nYou can also use `.hd <image URL>`.'
            }, { quoted: message });
          }

          await sock.sendMessage(chatId, { text: '🖼️ *Enhancing image to HD...* Please wait.' }, { quoted: message });
          ...
          // Upscale up to 2x, while avoiding unnecessarily huge WhatsApp media.
          const scale = Math.min(2, Math.max(1, 2048 / Math.max(width, height)));
          const outWidth = Math.max(width, Math.round(width * scale));
          const outHeight = Math.max(height, Math.round(height * scale));

          const output = await sharp(input)
            .resize({ width: outWidth, height: outHeight, fit: 'inside', kernel: sharp.kernel.lanczos3, withoutEnlargement: false })
          ...
          await sock.sendMessage(chatId, {
            image: output,
            caption: `✨ *HD Image Ready*\n📐 ${outWidth} × ${outHeight}`
          }, { quoted: message });
        } catch (error) {
          console.error('[HD] Error:', error.message);
          await sock.sendMessage(chatId, {
            text: `❌ *HD failed:* ${error.message}\n\nMake sure you reply to an image and try \.hd again.`
          }, { quoted: message });
        }
      }
    };
...
const VV_CONFIG = {
    ...
    retrievedMsg: '👁️ *View-Once Retrieved!*\n\n_Powered by lJUNAID MD 302 v7.2 ULTRA_ 🔥',
...
module.exports = {
  ...
  async handler(sock, message, args, context) {
    ...
    try {
      ...
      // Unknown sound: speak the supplied text instead of silently failing.
      if (!soundEffects.soundFiles[command] && !command.startsWith('gaali') && command !== 'custom') {
...
async function generateTTS(text, emotion = 'default') {
  try {
    // Reliable lightweight fallback: Google Translate TTS.
    // This returns a directly playable MP3 URL and does not require an API key.
...
module.exports = {
  ...
  async handler(sock, message, args, context) {
    ...
    try {
      ...
      // Hosted sound sites can occasionally be unavailable. Fall back to a
      // directly playable voice line so .sound2 still works.
      const fallbackText = `${sound.label.replace(/^[^A-Za-z0-9]+\s*/, '')}`;
      try {
        const q = encodeURIComponent(fallbackText);
        const ttsUrl = `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=ur&q=${q}`;
        await sock.sendMessage(chatId, {
          audio: { url: ttsUrl },
          mimetype: 'audio/mpeg',
          ptt: true,
        ...
        await sock.sendMessage(chatId, {
          text: `❌ Sound service unavailable: ${e.message}`,
          ...channelInfo
        }, { quoted: message });
      }
    }
