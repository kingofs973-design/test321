...
const channelInfo = {
  contextInfo: {
    ...
    forwardedNewsletterMessageInfo: {
      // Always use the currently configured JUNAID channel, never the old REDXBOT channel.
      newsletterJid: process.env.NEWSLETTER_JID || '120363408055942569@newsletter',
      newsletterName: process.env.NEWSLETTER_NAME || 'JUNAID MD 302',
      serverMessageId: -1
    }
  }
};
...
async function handleMessages(sock, messageUpdate) {
  try {
    ...
    if (message.message?.buttonsResponseMessage) {
      ...
      // TikTok .tiktok format picker: Music / Video buttons.
      if ((buttonId === 'tiktok_audio' || buttonId === 'tiktok_video') && global._handleTikTokButton) {
        await global._handleTikTokButton(sock, message, buttonId === 'tiktok_audio' ? 'audio' : 'video', { chatId, senderId, isGroup, channelInfo });
        return;
      }
