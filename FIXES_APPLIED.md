# 🔧 JUNAID MD 302 — Bug Fixes & Permission System Overhaul
...
- Anti-link behavior fixed: default is enabled in kick mode; enabled legacy warn/delete configs are migrated to kick; detected link is deleted first, then sender is immediately removed.
- Anti-link participant resolution now matches @lid/@s.whatsapp.net/phoneNumber metadata to avoid removing the wrong JID.

- Added complete dynamic board menu aliases: `.menu`, `.allmenu`, `.commands`, and `.list`; the menu is built from all commands currently registered in `lib/commandHandler.js`.

- `.me` added: plays one of 10 uploaded sounds randomly; `.me 1` through `.me 10` selects a specific sound.
